...
async def _seed_user(session, username, password="password123", role="investigator", clearance="SECRET"):
    u = User(username=username, hashed_password=hash_password(password), role=role, clearance=clearance)
    session.add(u)
    await session.flush()
    return u
...
async def test_case_document_version_verify_flow(session, client):
    ...
    # Create case.
    resp = await client.post("/cases", json={"title": "FIR-100", "classification": "CONFIDENTIAL"}, headers=headers)
    ...
    resp = await client.post(
        ...
        files={"file": ("statement.txt", b"The witness saw a blue sedan at midnight.", "text/plain")},
        headers=headers,
    )
    assert resp.status_code == 201, resp.text
    doc_id = resp.json()["id"]
    ...
    resp = await client.post(
        ...
        files={"file": ("v2.txt", b"The witness saw a blue sedan at midnight. Plate XYZ-123.", "text/plain")},
        headers=headers,
    )
...
async def test_cross_user_document_forbidden(session, client):
    ...
    resp = await client.post(
        "/cases", json={"title": "private", "classification": "CONFIDENTIAL"},
    ...
    for url in (f"/documents/{doc_id}", f"/documents/{doc_id}/versions/1/verify", f"/documents/{doc_id}/versions/1"):
