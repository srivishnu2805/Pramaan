...
async def test_ingestion_pdf_end_to_end(session):
    ...
    chunks = (await session.execute(select(Chunk).where(Chunk.document_id == doc.id))).scalars().all()
    assert len(chunks) >= 1
...
async def test_malformed_pdf_quarantined_or_failed(session):
    ...
    doc = await create_document(session, alice, case.id, "notes.txt", "CONFIDENTIAL", b"plain text notes")
    await enqueue_ingestion(session, doc.id)
    assert await process_next_job(session) is True
    job = (await session.execute(select(IngestionJob).where(IngestionJob.document_id == doc.id))).scalars().one()
...
async def test_executable_upload_rejected(session):
    ...
    session.add(case)
    await session.flush()
    doc = await create_document(session, alice, case.id, "evil.pdf", "CONFIDENTIAL", b"MZ\x90\x00evil-binary")
    await enqueue_ingestion(session, doc.id)
    assert await process_next_job(session) is True
    job = (await session.execute(select(IngestionJob).where(IngestionJob.document_id == doc.id))).scalars().one()
    assert job.status in ("QUARANTINED", "FAILED")
    refreshed = (await session.execute(select(Document).where(Document.id == doc.id))).scalars().one()
